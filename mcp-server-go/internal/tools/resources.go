    ...
    <div class="max-w-3xl mx-auto">
        ...
        <div class="mb-12 px-2">
            ...
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h1 class="text-2xl font-bold tracking-tight">Project Evolution</h1>
                    <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">Timeline of decisions and changes</p>
                </div>
                <!-- Actions -->
...
def generate():
    ...
    try:
        ...
        for row in cur.fetchall():
            d = dict(row)
            # 🔧 FIX: 不要覆盖原始 timestamp，如果为空则保留空字符串或使用 id 作为排序依据
            # 这样可以保留数据的真实时间信息
            if not d['timestamp']:
                # 如果 timestamp 确实为空，可以用 created_at 或 id 来推断
                # 但不要用 datetime.now()，因为每次运行都会变
                d['timestamp'] = d.get('created_at') or '1970-01-01 00:00:00'
            data.append(d)
        
        with open(OUTPUT_FILE, 'w', encoding='utf-8') as f:
            f.write(HTML_TEMPLATE.replace("__DATA_PLACEHOLDER__", json.dumps(data, ensure_ascii=False)))
